...
// 	fgets(readValue, 2000, senderFile);
	...
	float zeroPercent = (float) zeroToZero/(float)(zeroToZero + zeroToOne);
	float onePercent = (float) oneToOne/ (float) (oneToOne + oneToZero);
	float totalPercent = (.5*zeroPercent) + (.5*onePercent);
	if(totalPercent > 0) {
		...
		totalZeroSent = totalZeroSent - zeroToZero - zeroToOne;
		totalOneSent = totalOneSent - oneToOne - oneToZero;
	}
	fclose(senderFile);
	fclose(receiverFile);
    }
