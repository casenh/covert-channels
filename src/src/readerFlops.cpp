...
/* Initialize the PAPI library and create the events that we wish to monitor */
    ...
    if(PAPI_add_event(eventSet, PAPI_FP_OPS) != PAPI_OK){ handle_error(1); }
...
//     cout << "Working..." << endl;
	...
	/*if(counters[0] < (average - 15) && count >= 300) //Prevent outliers from scewing the data
	{
	    counters[0] = (average - 15);
	}
	if(counters[0] > (average + 15) && count >= 300)
	{
	    counters[0] = average + 15;
	}*/
